# Equipo de trabajo

## Proyecto

- Nombre provisional: El proyecto 404
- Organización: Liceo Antonio Varas de la Barra
- Fecha: 31-07-2026

## Integrantes y roles

| Integrante | Rol principal | Fortalezas | Responsabilidad inicial |
|---|---|---|---|
| Alejandra Mamani| Analistas de Requerimientos| Analitica | Recopila, identifica y documenta información y necesidades|
| Francisca Calle| Diseñador UX/UI| Creativa y curiosa| Crea y diseña bocetos definiendo la estructura de la pagina web|
| Liseth Capia| Lider del equipo | Responsable y liderazgo| Organiza al equipo, asiga roles, responsabilidades y planifica las tareas del proyecto|
| Isabel Fernández| Desarrollador de Frontend| Responsable y Conocimiento | Desarrolla la parte visual de la pagina web|
| Beymar Quispe| Desarrollador Backend y Base de Datos| Directo | Desarrolla y Administra la base de datos de estudiantes|

## Normas internas

- Canal de comunicación: WhatsApp
- Horario de coordinación: Martes, Miercoles, Viernes de las 08:00 am hasta las 04:50 pm.
- Forma de tomar decisiones: tomar en cuenta la opinion de todos y llegar a una solución que aproben todos.
- Procedimiento ante inasistencia: recurrir ante jefe o lider del equipo e informar al profesor jefe justificando la causa.
- Procedimiento ante tareas atrasadas: con la ayuda del equipo terminaríamos el procedimiento de la tarea pendiente lo antes posible.

## Compromisos

- [x] Todos tendrán tareas verificables.
- [x] Todos participarán en la presentación.
- [ ] Se utilizarán issues y pull requests.
- [x] Se respetarán los datos de la organización.
- [x] Se registrará el uso importante de IA.

## Aprobación del equipo

| Integrante | Confirmación |
|---|---|
| Alejandra Mamani| si |
| Francisca Calle| si |
| Liseth Capia| si |
| Isabel Fernandez| si |
| Beymar Quispe| si |
